...
class CVRPTrainer:
    ...
    def __init__(self,
        ...
        self.reptile_first, self.reptile_last = [], []
        # self.reptile2_first, self.reptile2_last, self.reptile4_first, self.reptile4_last = [], [], [], []
        # self.reptile6_first, self.reptile6_last, self.reptile8_first, self.reptile8_last = [], [], [], []
        # self.reptile10_first, self.reptile10_last = [], []
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, self.meta_params['epochs'] + 1):
            ...
            if epoch % 1000 == 0:
                torch.save({
                    'reptile_first': self.reptile_first,
                    'reptile_last': self.reptile_last,
                }, "./grad_dir_cos.pt")
    ...
    def _train_one_epoch(self, epoch):
        ...
        # compute cos sim
        cos1 = self.cos(maml_first_layer.view(-1), reptile_first_layer.view(-1))
        cos2 = self.cos(maml_last_layer.view(-1), reptile_last_layer.view(-1))
        self.reptile_first.append(cos1.item())
        self.reptile_last.append(cos2.item())
